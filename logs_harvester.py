...
def normalize_event(evt):
    ...
    """
    result = {"timestamp": evt.get("timestamp"), "text": evt.get("text")}
...
def main():
    ...
    parser.add_argument("--filter", dest="filters", action="append", default=[],
                         help="field=value (CONTAINS) or field==value (EQ). Repeatable, ANDed. "
                              "Use 'namespace:fieldname' for content-pack extracted fields.")
    ...
    parser.add_argument("--ip-field", default="text",
                         help="Field to search the resolved --vm IP in (default: 'text', full-text search).")
    parser.add_argument("--src", default=None,
                         help="VM shortname, FQDN, or literal IP -- keep only events whose extracted "
                              "'source' field matches it. Requires a preset or --extract that produces a "
                              "'source' field (e.g. --preset nsx-dfw). Applied after fetching/extraction, "
                              "as a precise client-side filter; a broad server-side pre-filter on the IP is "
                              "also added automatically to reduce what's fetched.")
    parser.add_argument("--dst", default=None,
                         help="Same as --src, but matches the extracted 'destination' field instead.")
    ...
    if args.vm:
        try:
            ip = resolve_vm_to_ip(args.vm, args.domain, args.dns_server)
        except RuntimeError as e:
            print(f"DNS resolution failed: {e}", file=sys.stderr)
            sys.exit(1)
        filters.append((args.ip_field, "CONTAINS", ip))
    ...
    if src_ip:
        before = len(events)
        events = [e for e in events if ip_matches_field(e.get("source"), src_ip)]
        print(f"--src filter: {before} -> {len(events)} events", file=sys.stderr)
    if dst_ip:
        before = len(events)
        events = [e for e in events if ip_matches_field(e.get("destination"), dst_ip)]
        print(f"--dst filter: {before} -> {len(events)} events", file=sys.stderr)
