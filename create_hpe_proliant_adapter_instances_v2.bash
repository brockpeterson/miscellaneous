...
for i in `cat /users/brockp/scripts/ilos.txt`
	...
	jq -r '.collectorGroups | to_entries[] | "\(.key + 1). \(.value.name) : collectorId \(.value.collectorId[0])"' "/tmp/b.out"
	echo

	# Ask user which Collector Group they would like to use
	read -p "Enter the number of the Collector Group you want to use: " choice
